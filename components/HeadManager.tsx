...
export default function HeadManager() {
  ...
  // TikTok Pixel - Injeção Direta no DOM
  useEffect(() => {
    if (!mounted || typeof window === 'undefined') return;

    // Desabilitar no desenvolvimento
    if (isDevelopment) {
      console.log('🔧 [HeadManager] Modo desenvolvimento: TikTok Pixel desabilitado');
      return;
    }

    // Remover script antigo se existir
    const oldTikTokScript = document.getElementById('tiktok-pixel-script');
    if (oldTikTokScript) oldTikTokScript.remove();

    console.log('📦 [HeadManager] Injetando TikTok Pixel...');
    ...
    console.log('✅ [HeadManager] TikTok Pixel injetado com sucesso!');

    // Cleanup
    return () => {
      const tiktok = document.getElementById('tiktok-pixel-script');
      if (tiktok) tiktok.remove();
    };
  }, [mounted, pathname, isDevelopment]);
